  ...
  async.series({
    ...
    createInDB: function(cb) {
      self.db.create(function(err) {
        if (!!err) {
          self.logger.error('create in db failed:', err, self.toJSON());
        } else {
          self.mem.isLoaded = self.db.isSaved = true;
          self.db.p(self.mem.p());
          self.logger.debug('create in db success:', self.toJSON());
        }
        cb(err);
      });
    },
    ...
    createInCache: function(cb) {
      self.cache.create(function(err) {
        if (!!err) {
          self.logger.error('create in cache failed:', err, self.toJSON());
        } else {
          self.cache.isSaved = true;
          self.cache.p(self.mem.p());
          self.logger.debug('create in cache success:', self.toJSON());
        }
        cb(err);
      });
    }
  ...
  async.parallel({
    removeFromDB: function(cb) {
      self.db.remove(function(err) {
        if (!!err) {
          self.logger.error('remove from db failed:', err, self.toJSON());
        } else {
          self.mem.isLoaded = self.db.isSaved = false;
          self.logger.debug('remove from db success:', self.toJSON());
        }
        cb(err);
      });
    },
    ...
    removeFromCache: function(cb) {
      self.cache.remove(function(err) {
        if (!!err) {
          self.logger.error('remove from cache failed:', err, self.toJSON());
        } else {
          self.cache.isSaved = false;
          self.logger.debug('remove from cache success:', self.toJSON());
        }
        cb(err);
      })
    }
  ...
  async.series({
    ...
    loadFromCache: function(cb) {
      ...
      self.cache.load(function(err, res) {
        if (!!err) {
          self.logger.error('load from cache failed: ', err, self.toJSON());
        } else {
          if (!!res) {
            self.p(res);
            self.db.p(res);
            ...
            self.cache.isSaved = true;
            self.logger.debug('load from cache success:', self.toJSON());
          } else {
            self.logger.debug('load from cache failed: miss', self.toJSON());
          }
        }
        cb(err);
      });
    ...
    loadFromDB: function(cb) {
      ...
      self.db.load(function(err, res) {
        if (!!err) {
          self.logger.error('load from db failed: ', err, self.toJSON());
        } else {
          if (!!res) {
            self.p(res);
            self.mem.isLoaded = self.db.isSaved = true;
            self.logger.debug('load from db success: ', self.toJSON());
          } else {
            self.logger.debug('load from db failed: miss', self.toJSON());
          }
        }
        cb(err);
      });
  ...
  async.parallel({
    updateDB: function(cb) {
      self.db.update(function(err) {
        if (!!err) {
          self.logger.error('update db failed:', err, self.toJSON());
        } else {
          self.p(self.toJSON());
          self.logger.debug('update db success:', self.toJSON());
        }
        cb(err);
      });
    },
    ...
    updateCache: function(cb) {
      self.cache.update(function(err) {
        if (!!err) {
          self.logger.error('update cache failed:', err, self.toJSON());
        } else {
          self.logger.debug('update cache success:', self.toJSON());
        }
        cb(err);
      });
    }
