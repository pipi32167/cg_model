  ...
  for (prop in props) {
    if (props.hasOwnProperty(prop)) {
      if (props[prop].type === consts.PropType.MODEL) {
        ...
        Model = lib.getModel(props[prop].modelName);
...
pro.find = function(args, cb) {
  ...
  self.db.find(args, function(err, res) {
    if (!!err) {
      lib.logger.error('find failed: ', err, self.def.name);
      cb(err);
      return;
    }

    lib.logger.debug('find success: ', self.def.name, ', find args:', args, ', result:', res);
    cb(null, res);
  });
}
...
pro.findAll = function(cb) {
  ...
  self.db.findAll(function(err, res) {
    if (!!err) {
      lib.logger.error('find all failed: ', err, self.def.name);
      cb(err);
      return;
    }

    lib.logger.debug('find all success: ', self.def.name, ', result:', res);
    cb(null, res);
  });
}
  ...
  async.series({
    ...
    removeFromDB: function(cb) {
      self.db.remove(args, function(err, res) {
        if (!!err) {
          lib.logger.error('remove from db failed:', err, args);
        } else {
          primaryValues = res;
          lib.logger.debug('remove from db success', args);
        }
        cb(err);
      })
    },
    ...
    removeFromCache: function(cb) {
      self.cache.remove(primaryValues, function(err) {
        if (!!err) {
          lib.logger.error('remove from cache failed:', err, args);
        } else {
          lib.logger.debug('remove from cache success', args);
        }
        cb(err);
      });
    }
  ...
  async.series({
    removeFromDB: function(cb) {
      self.db.removeAll(function(err) {
        if (!!err) {
          lib.logger.error('remove all from db failed:', err);
        } else {
          lib.logger.debug('remove all from db success');
        }
        cb(err);
      });
    },
    ...
    removeFromCache: function(cb) {
      self.cache.removeAll(function(err) {
        if (!!err) {
          lib.logger.error('remove all from cache failed:', err);
        } else {
          lib.logger.debug('remove all from cache success');
        }
        cb(err);
      });
    }
...
pro.count = function(args, cb) {
  this.db.count(args, function(err, res) {
    if (!!err) {
      lib.logger.error('count from db failed:', err, args);
    } else {
      lib.logger.debug('count from db success', args);
    }
    cb(err, res);
  });
}
...
pro.countAll = function(cb) {
  this.db.countAll(function(err, res) {
    if (!!err) {
      lib.logger.error('count all from db failed:', err);
    } else {
      lib.logger.debug('count all from db success');
    }
    cb(err, res);
  });
}
