...
exp.startCronJob = function(type) {
  if (cronJobs[type]) {
    exp.logger.debug('start cron job error: already started', type);
    return;
  }
...
exp.stopCronJob = function(type) {
  if (!cronJobs[type]) {
    exp.logger.debug('stop cron job error: already stopped', type);
    return;
  }
  exp.logger.debug('stop cron job:', type);

  if (type === DataType.MYSQL_LATE) {
    require('./data/data_mysql_late').stopCronJob();
    cronJobs[type] = false;
  }
}
