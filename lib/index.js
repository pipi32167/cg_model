...
exp.createModel = function(def) {
  ...
  var Model = models[def.name] = function() {
    ...
    this.db = new DataDB(this, clients.db[def.db.db_name]);
    this.cache = new DataCache(this, clients.cache[def.cache.cache_name]);
  ...
  var DataDB = Model.db = Data.factory(def.db.type)(Model, clients.db[def.db.db_name]);
  var DataCache = Model.cache = Data.factory(def.cache.type)(Model, clients.cache[def.cache.cache_name]);
  assert.ok(!!DataMem);
  assert.ok(!!DataDB);
  assert.ok(!!DataCache);
...
exp.setDBClient = function(name, dbClient) {
  clients.db[name] = dbClient;
}

exp.setCacheClient = function(name, cacheClient) {
  clients.cache[name] = cacheClient;
}
