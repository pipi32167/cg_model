...
module.exports = function(lib, def, connType) {
  ...
  var convertLoadValue = function(type, value) {
    ...
    //already convert to the specified type
  ...
  DataRedis.prototype.load = function(cb) {
    ...
    async.series({
      load: function(cb) {
        ...
        self.conn.hgetall(hashKey, function(err, res) {
          if (!err) {
            data = res;
          }
          cb(err);
      ...
      upgrade: function(cb) {
        ...
        self.upgrade(data, cb);
      },

      convert: function(cb) {
        if (!!data) {
          for (var prop in props) {
            if (props.hasOwnProperty(prop) && data.hasOwnProperty(prop)) {
              data[prop] = convertLoadValue(props[prop].type, data[prop]);
            }
          }
        }
        cb();
      ...
      if (data && data.__version !== undefined) {
  ...
  DataRedis.prototype.__update = function(cb) {
    ...
    if (def.version !== undefined) {
      updateArgs.push('__version');
      updateArgs.push(def.version);
    }
