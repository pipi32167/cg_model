...
module.exports = function(lib, def, connType) {
  ...
  DataRedis.prototype.genHashKey = function() {
    ...
    var values = _(primaryKeys).map(function(elem) {
      var value = self.model.p(elem);
      if (value === undefined) {
        var err = _s.sprintf('model[%s] generate hash key need all primary key exists: %s', self.model.def.name, elem);
        assert.ok(false, err);
      }
  ...
  DataRedis.prototype.convertUpdateValue = function(prop, type, value) {
    switch (type) {
      ...
      case 'number':
        if (!_.isNumber(value)) {
          assertConvertUpdateValue(this.model, prop, type, value);
        }
        return String(value);
      case 'bool':
  ...
  DataRedis.prototype.__update = function(cb) {
    ...
    var updateArgs = _(props)
      ...
      .keys()
      .map(function(elem) {
        var res = self.convertUpdateValue(elem, props[elem].type, model.p(elem));
        return [elem, res];
      })
      .flatten()
      .value();
