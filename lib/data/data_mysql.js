...
var getConnection = function(lib, def, connType) {
...
module.exports = function(lib, def, connType) {
  ...
  DataMySql.conn = getConnection(lib, def, connType);
  ...
  DataMySql.prototype.getDBConn = function() {
    if (!this.__conn) {
      this.__conn = getConnection(lib, def, connType);
    }
    return this.__conn;
  }
  ...
  var __queryBatch = function(dbObjs, createQueryFn, querySuccessFn, cb) {
    ...
    var q = dbObjs[0].conn.query(batchQuery.text, batchQuery.values, function(err, results) {
      if (!!err) {
        lib.logger.error('batchLoad error:', err, sqlFormat(q.sql, q.values));
      } else {
        lib.logger.debug('batchLoad success');
        var elem;
  ...
  DataMySql.prototype.afterUpdateSuccess = function() {
