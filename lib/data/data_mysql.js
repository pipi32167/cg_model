...
module.exports = function(app, Model, connType) {
   ...
   */
  var convertUpdateValue = function(type, value) {
    switch (type) {
      case 'date':
      ...
      case 'object':
        ...
        return JSON.stringify(value);
      case 'number':
        assert.ok(_.isNumber(value), typeof value);
        return value;
      case 'bool':
        assert.ok(_.isBoolean(value), typeof value);
        return value;
      case 'string':
        assert.ok(_.isString(value), typeof value);
        return value;
      default:
        assert.ok(false, 'unsupported type:' + type);
    }
  ...
  DataMySql.prototype.update = function(cb) {
    ...
    var props = this.model.def.props;
    for (var prop in updateArgs) {
      if (updateArgs.hasOwnProperty(prop)) {
        updateArgs[prop] = convertUpdateValue(props[prop].type, updateArgs[prop]);
      }
    }
    var whereArgs = this.model.getPrimaryKeysAndValues();
    var query = queryBuilder.update(updateArgs).where(whereArgs).toQuery('mysql');
    var self = this;
    this.conn.query(query.text, query.values, function(err) {
      if (!err) {
