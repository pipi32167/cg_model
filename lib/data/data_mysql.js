...
var createUpdateArgs = function(Model, queryBuilder, args) {
  var res = {};
  var props = Model.def.props;
  ...
  for (prop in updateArgs) {
    if (updateArgs.hasOwnProperty(prop)) {
      if (!isSqlKey(prop)) {
        if (!props[prop]) {
          var err = _s.sprintf('model[%s] doesn\'s contains property[%s]', Model.def.name, prop);
          assert.ok(false, err);
        }
        res[prop] = convertUpdateValue(Model.def.name, prop, props[prop].type, updateArgs[prop]);
      }
    }
  }
  return res;
...
module.exports = function(lib, Model, connType) {
  ...
  DataMySql.prototype.createSync = DataMySql.prototype.create = function(cb) {
    ...
    var q = this.conn.query(query.text, query.values, function(err, res) {
      if (!!err) {
        self.lib.logger.error('create error:', err, q.sql);
      } else {
  ...
  var __queryBatch = function(dbObjs, createQueryFn, querySuccessFn, cb) {
    if (dbObjs.length === 0) {
      cb();
      return;
    }
    ...
    var query, i, j;
    for (i = 0; i < dbObjs.length; i++) {
      // query = dbObjs[i].createLoadQuery();
      query = createQueryFn.call(dbObjs[i]);
    ...
    var q = dbObjs[0].conn.query(batchQuery.text, batchQuery.values, function(err, results) {
      if (!!err) {
        model.logger.error('batchLoad error:', err, sqlFormat(q.sql, q.values));
      } else {
        model.logger.debug('batchLoad success');
        var elem;
        for (var i = 0; i < dbObjs.length; i++) {
          ...
          try {
            if (dbObjs.length === 1) {
              // elem.afterLoadSuccess(results);
              querySuccessFn.call(elem, results);
            } else {
              // elem.afterLoadSuccess(results[i]);
              querySuccessFn.call(elem, results[i]);
            }
          } catch (e) {
  ...
  DataMySql.prototype.batchCreateSync = function(dbObjs, cb) {
    __queryBatch.call(
      this,
      dbObjs,
      DataMySql.prototype.createCreateQuery,
      DataMySql.prototype.afterCreateSuccess,
      cb);
  }
  ...
  DataMySql.prototype.afterLoadSuccess = function(res) {
    res = res[0];
    if (!!res) {
      var props = this.model.def.props;
      for (var prop in res) {
  ...
  DataMySql.prototype.load = function(cb) {
    ...
    var q = this.conn.query(query.text, query.values, function(err, res) {
      if (!!err) {
        self.lib.logger.error('load error:', err, q.sql);
      } else {
        self.afterLoadSuccess(res);
      }
      cb(err);
    });
    this.lib.logger.debugSql(q);
  ...
  DataMySql.prototype.batchLoad = function(dbObjs, cb) {
    __queryBatch.call(
      this,
      dbObjs,
      DataMySql.prototype.createLoadQuery,
      DataMySql.prototype.afterLoadSuccess,
      cb);
  }
  ...
  DataMySql.prototype.update = function(cb) {
    ...
    var q = this.conn.query(query.text, query.values, function(err) {
      if (!!err) {
        self.lib.logger.error('update error:', err, q.sql);
      } else {
  ...
  DataMySql.prototype.remove = function(cb) {
    ...
    var q = this.conn.query(query.text, query.values, function(err) {
      if (!!err) {
        self.lib.logger.error('remove error:', err, q.sql);
      } else {
        self.isSaved = false;
      }
      cb(err);
  ...
  DataMySql.find = function(args, cb) {
    ...
    var q = this.conn.query(query.text, query.values, function(err, res) {
      if (!!err) {
        self.lib.logger.error('static find error:', err, q.sql);
      }
      cb(err, res);
    });
    this.lib.logger.debugSql(q);
    ...
    async.series({
      ...
      remove: function(cb) {
        ...
        var q = self.conn.query(query.text, query.values, function(err) {
          if (!!err) {
            self.lib.logger.error('static remove error:', err, q.sql);
          }
          cb(err);
        });
        self.lib.logger.debugSql(q);
      }
    }, function(err) {
  ...
  DataMySql.findAll = function(cb) {
    ...
    var q = this.conn.query(query.text, query.values, function(err, res) {
      ...
      if (!!err) {
        self.lib.logger.error('static findAll error:', err, q.sql);
      }
      cb(err, res);
    });
    this.lib.logger.debugSql(q);
  ...
  DataMySql.removeAll = function(cb) {
    ...
    var q = this.conn.query(sql, [], function(err) {
      if (!!err) {
        self.lib.logger.error('static removeAll error:', err, q.sql);
      }
      cb(err);
    });
    this.lib.logger.debugSql(q);
  ...
  DataMySql.update = function(args, cb) {
    ...
    var updateArgs = createUpdateArgs(Model, queryBuilder, args.$update);
    var whereArgs = createWhereArgs(Model, queryBuilder, args.$where || {});
    var query = queryBuilder.update(updateArgs).where(whereArgs).toQuery('mysql');
    var self = this;
    var q = this.conn.query(query.text, query.values, function(err) {
      if (!!err) {
        self.lib.logger.error('static update error:', err, q.sql);
      }
      cb(err);
    });
    this.lib.logger.debugSql(q);
