...
var exp = module.exports = function(lib, def, connType) {
	...
	DataMySqlShard.prototype.load = function(cb) {
		addLoadJob(this);
		this.once(consts.Event.LOADED, function (err) {
			cb(err);
		});
	}
...
util.inherits(LoadJob, JobMySqlLoad);

LoadJob.prototype.createQuery = function(job) {
	var query;
	query = job.createLoadQuery();
	return query;
}
...
var addLoadJob = function(job) {
	if (!loadJobs[job.dbName]) {
		loadJobs[job.dbName] = new LoadJob();
	}
	loadJobs[job.dbName].add(job);
}
