...
var defaultGetShardIndex = function() {

	assert.ok(this.shardCount > 0);
	var str = this.getShardValue().toString();
	return crc.crc32(str) % this.shardCount;
}
...
var exp = module.exports = function(lib, def, connType) {
	...
	var DataMySqlShard = function(lib, model) {
		...
		this.shardCount = getShardCount(lib, def);
		this.isShard = isShard(lib, def);
	}
	...
	DataMySqlShard.prototype.getDBName = function() {
		if (!this.__dbName) {
			if (!this.isShard) {
				...
				this.__dbName = this.getShardDBName();
			}
		}
	...
	var __getShardIndex;
	if (def.db.shard_strategy instanceof(Function)) {
		__getShardIndex = def.db.shard_strategy;
	} else if (lib.get('shard_strategy') instanceof(Function)) {
		__getShardIndex = lib.get('shard_strategy');
	} else {
		__getShardIndex = defaultGetShardIndex;
	}

	DataMySqlShard.prototype.getShardIndex = __getShardIndex;

	DataMySqlShard.prototype.getShardDBName = function() {
		if (this.isShard) {
			return shardDBNames[this.getShardIndex()];
		} else {
			return this.getMainDBName();
		}
	};

	DataMySqlShard.prototype.getShardKey = function() {
		...
		for (var prop in props) {
			if (props.hasOwnProperty(prop) && props[prop].shard) {
				if (props[prop].type !== consts.PropType.NUMBER &&
					...
					assert.ok(false, 'shard key\'s type should only be string or number, bug got ' + props[prop].type + ' ' + def.name);
				}
				return prop;
			}
		}
		assert.ok(false, 'please define shard key: ' + def.name);
	}

	DataMySqlShard.prototype.getShardValue = function() {
		var key = this.getShardKey();
		var res = this.model.p(key);
		if (res === undefined || res === null) {
			assert.ok(false, 'shard key should not be empty:' + key);
		}
		return res;
	}
	...
	var __updateSync = DataMySqlShard.prototype.updateSync;
	DataMySqlShard.prototype.updateSync = function(cb) {
		...
		if (!updateImmediate) {
			...
			addUpdateSyncJob(this);
			this.once(consts.Event.UPDATED, cb);
		}
	}

	DataMySqlShard.prototype.update = function(cb) {
	...
	DataMySqlShard.find = function(args, cb) {
		...
		shardBatchDoQuery(query, function(err, results) {
			...
			if (args.$limit &&
				(!args.$shardOpts ||
					args.$shardOpts && !args.$shardOpts.noLimitOnReduce)) {
	...
	return require('bluebird').promisifyAll(DataMySqlShard);
}
...
var addUpdateSyncJob = function(job) {
	var jobName = utils.getJobName(consts.DataType.MYSQL_SHARD, job.dbName, 'updateSync');
	lib.getJob(jobName).add(job);
}
